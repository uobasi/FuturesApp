...
def plotChart(df, lst2, num1, num2, x_fake, df_dx, optionOrderList, stockName='', prevdtstr:str='', sord:list=[], trends:list=[], lstVwap:list=[], bigOrders:list=[], pea:bool=False, timeStamp:int=None, previousDay:bool=False, OptionTimeFrame:list=[], overall:list=[]):
    ...
    optColor = [     'rgba(0,128,0,0.25)' if float(i[2]) > float(i[3])
                else 'rgba(255,0,0,0.25)' if float(i[3]) > float(i[2])
                else 'rgba(128,128,128,0.25)' if float(i[3]) == float(i[2])
    ...
    fig.add_trace(
        go.Bar(
            ...
            #orientation='h',
            #width=0.2,
            marker_color= [  'rgba(255,0,0,0.25)' if float(i[2]) > float(i[3])
                        else 'rgba(0,128,0,0.25)' if float(i[3]) > float(i[2])
                        else 'rgba(128,128,128,0.25)' if float(i[3]) == float(i[2])
                        else i for i in OptionTimeFrame],
            hovertext=pd.Series([i[0]+' '+i[1] for i in OptionTimeFrame]),
    ...
    #if 2 in lstVwap:
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_2'], mode='lines', opacity=0.20, name='UPPERVWAP2', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N2'], mode='lines', opacity=0.20, name='LOWERVWAP2', line=dict(color='black')))
    #if 0 in lstVwap:
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_25'], mode='lines', opacity=0.20, name='UPPERVWAP2.5', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N25'], mode='lines', opacity=0.20, name='LOWERVWAP2.5', line=dict(color='black')))
    #if 1 in lstVwap:    
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_1'], mode='lines', opacity=0.20, name='UPPERVWAP1', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N1'], mode='lines', opacity=0.20, name='LOWERVWAP1', line=dict(color='black')))
        
    #if 1.5 in lstVwap:     
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_15'], mode='lines', opacity=0.20, name='UPPERVWAP1.5', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N15'], mode='lines', opacity=0.20, name='LOWERVWAP1.5', line=dict(color='black')))

    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_0'], mode='lines', opacity=0.20, name='UPPERVWAP0.5', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N0'], mode='lines', opacity=0.20, name='LOWERVWAP0.5', line=dict(color='black')))
    ...
    indsAbove = [i for i in OptionTimeFrame if round(i[6],2) > 0.65 and int(i[4]) < len(df) and float(i[2]) >= (sum([i[2]+i[3] for i in OptionTimeFrame]) / len(OptionTimeFrame))] # and int(i[4]) < len(df) [(len(df)-1,i[1]) if i[0] >= len(df) else i for i in [(int(i[10]),i[1]) for i in sord if i[11] == stockName and i[1] == 'AboveAsk(BUY)']]
    
    indsBelow = [i for i in OptionTimeFrame if round(i[7],2) > 0.65 and int(i[4]) < len(df) and float(i[3]) >= (sum([i[3]+i[2] for i in OptionTimeFrame]) / len(OptionTimeFrame))] # and int(i[4]) < len(df) imbalance = [(len(df)-1,i[1]) if i[0] >= len(df) else i for i in [(i[10],i[1]) for i in sord if i[11] == stockName and i[13] == 'Imbalance' and i[1] != 'BelowBid(SELL)' and i[1] != 'AboveAsk(BUY)']]
...
def update_graph_live(n_intervals, data):
    ...
    if stkName in symbolNameList: 
        fg = plotChart(df, [hs[1],ntList[:3]], va[0], va[1], [], [], bigOrders=[], optionOrderList=[], stockName=symbolNameList[symbolNumList.index(symbolNum)], previousDay=False, prevdtstr='', pea=False, sord = [], OptionTimeFrame = timeFrame, overall=[]) #trends=FindTrends(df,n=10)
    elif stkName in currSymbolNameList:
        fg = plotChart(df, [hs[1],ntList[:3]], va[0], va[1], [], [], bigOrders=[], optionOrderList=[], stockName=currSymbolNameList[currSymbolNumList.index(symbolNum)], previousDay=False, prevdtstr='', pea=False, sord = [], OptionTimeFrame = timeFrame, overall=[]) #trends=FindTrends(df,n=10)
